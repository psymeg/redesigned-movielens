...
If we convert the time stamps, we can see that the oldest review is dated `r minDate` and the most recent time stamp is `r maxDate`.
...
If we extract the release year from the title column we find that the earliest movie reviewed was released in `r minReleaseYear` and the most recently reviewed movie was released in `r maxReleaseYear`. This makes sense given the final review in the data set was received at `r maxFullDate`. During the time the dataset was collated there were 2 ways people were exposed to movies - at theaters and at home on video cassette. Theaters predominantly showed new releases with occasional film festivals and late night showings of classic films such as Clockwork Orange and The Rocky Horror Picture Show. Video Cassettes were rented from local video stores and were a mix of recent releases and classic films. 
...
# check if the same user effect (many ratings) is present in this code set as it was in unit 8
edx %>% 
  group_by(userId) %>% 
